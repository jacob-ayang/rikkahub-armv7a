...
编译 32 位 ARM V7a 版本，适合旧设备和预算型设备。

**触发条件：**
- 手动触发 (workflow_dispatch)
- 代码变更时自动触发（到 master 分支）
...
**输出：**
```
rikkahub-{版本号}-armv7-{时间戳}.apk
```

### 2. Build Universal (`.github/workflows/build-universal.yml`)

编译 64 位通用版本（arm64-v8a + x86_64），适合现代设备。

**特性同 Build ARM V7**

**输出：**
```
rikkahub-{版本号}-universal-{时间戳}.apk
```

### 3. Build All Variants (`.github/workflows/build-all.yml`)

同时编译所有版本（ARM V7 + Universal），并创建统一的 Release。
...
#### 1.2 编码 Keystore 为 Base64

**在 Linux/Mac 上：**
```bash
cat rikkahub.jks | base64 -w 0 > keystore_base64.txt
cat keystore_base64.txt  # 复制输出内容
```
...
**说明：**
- `storePassword`: 生成 keystore 时设置的主密码
...
**解决：**
1. 确认已添加三个 Secret：`KEY_BASE64`, `SIGNING_CONFIG`, `GOOGLE_SERVICES_JSON`
2. 确认 Secret 值非空
3. 检查 Secret 名称拼写是否完全一致
...
3. **Google Services：**
   - 不要在代码中硬编码 Firebase 配置
   - 从 Firebase Console 定期更新配置
   - 仅在 CI 环境中存储敏感信息
...
**A:** 满足不同的使用场景：
- **Build ARM V7**: 编译 32 位版本（旧设备）
- **Build Universal**: 编译 64 位版本（现代设备）
- **Build All**: 同时编译两个版本（完整发布）
...
**A:** 工作流中的 `SIGNING_CONFIG` Secret 包含签名信息，Gradle 自动读取并应用。
